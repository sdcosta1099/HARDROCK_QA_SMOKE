...
test('UH-9902 HALO|Check-In|[Y Y Y] Single Item | Booked In Advance - Attendance | attendance available | primary available', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-99024')
    //login steps

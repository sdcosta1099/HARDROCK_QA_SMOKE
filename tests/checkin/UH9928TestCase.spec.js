...
test('UH-9928 HALO | Check-In | [Y Y Y] Override Modify | Prizes Depleted from Master List | Capacity Available ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9928')
    //login steps

...
test('UH-9852 UH-9852 HALo|Check-In|Check In for Overridden Booking: Items: Yes | Attendance: No | Backup: No | Reserved Item NOT available | Multiple Items ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9852')
    //login steps

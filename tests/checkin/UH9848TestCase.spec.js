...
test('UH-9848  HALo|Check-In|Test|Override to Check In - Items: YES | Attendance: NO | Backup: NO - Multi item - Inv. Avail. - Booked in Advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9848')
    //login steps

...
test('UH-9874 HALo|Check In|Override to Check In - Items: No | Attendance: Yes | Backup: No - Not Booked in Advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9874')
    //login steps

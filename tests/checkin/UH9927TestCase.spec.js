...
test('UH-9927 HALO | Check-In | [Y Y Y] Override Modify | Prizes available in Master List | Capacity Available ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9927')
    //login steps

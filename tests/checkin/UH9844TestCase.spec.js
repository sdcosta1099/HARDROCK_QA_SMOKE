...
test('UH9844 HALo|Check-In|Test| Check In - Items: YES | Attendance: NO | Backup: NO - Single item - Inv. Avail. - Booked in Advance ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9844')
    //login steps

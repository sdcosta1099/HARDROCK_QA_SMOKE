...
test('UH-9895 HALO|Check-In|[Y Y Y] Single Item | Booked In Advance - Backup | fully committed | backup depleted', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9895')
    //login steps

...
test(' UH-9915 HALO|Check-In|[Y Y Y] Multi Item | Booked In Advance - Backup | attendance available | prizes available ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9915')
    //login steps

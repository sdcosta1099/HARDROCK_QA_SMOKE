...
test('UH-9883 HALo|Check-In|Override to Check In - Items: NO | Attendance: YES | Backup: YES - Capacity Avail. - Not booked in advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9883')
    //login steps

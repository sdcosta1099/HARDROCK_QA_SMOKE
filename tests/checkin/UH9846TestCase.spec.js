...
test('UH-9846 HALo|Check-In|Test|Override to Modify Check In - Items: YES | Attendance: NO | Backup: NO - Single Item ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9846')
    //login steps

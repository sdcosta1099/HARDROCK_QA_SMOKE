...
test('UH-9868 HALo|Check In for Overridden Booking - Items: Yes | Attendance: No | Backup: Yes - Single and Multi items', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9868')
    //login steps

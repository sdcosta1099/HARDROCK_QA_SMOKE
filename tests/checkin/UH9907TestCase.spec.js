...
test('UH-9907 HALO|Check-In|[Y Y Y] Single Item | Primary Depleted | Prebooked Primary | PrzMax > 1 CapMax > 1 ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9907')
    //login steps

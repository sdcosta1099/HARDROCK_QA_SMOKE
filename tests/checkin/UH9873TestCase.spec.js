...
test('UH-9873 HALo|Check-In|Test|Override to Check In - Items: No | Attendance: Yes | Backup: No - Patron not invited ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9873')
    //login steps

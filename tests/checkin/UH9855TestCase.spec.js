...
test('UH-9855 HALo|Check-In|Test|Override to Check In - Items: YES | Attendance: NO | Backup: NO - Single item - Patron not invited', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9855')
    //login steps

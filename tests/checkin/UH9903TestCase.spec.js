...
test(' UH-9903  HALO|Check-In|[Y Y Y] Single Item | Primary depleted | Prebooked Attendance | PrzMax = 1 CapMax = 1', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9903')
    //login steps

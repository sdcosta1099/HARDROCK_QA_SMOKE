...
test('UH-9856 HALo|Check In|Override to Check In - Items: YES | Attendance: NO | Backup: YES - Multi item - Patron not invited ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9856')
    //login steps

...
test('UH-9841 HALo | Check-In | Test | Check In - Items: YES | Attendance: NO | Backup: NO - Multiple items - No inventory available', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9841')
    //login steps

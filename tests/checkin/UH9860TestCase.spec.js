...
test('UH-9860 HALo | Check in | Override to Modify Check In - Items: YES | Attendance: NO | Backup: YES - Single Item ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9860')
    //login steps

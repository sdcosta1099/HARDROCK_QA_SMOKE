...
test('UH-9845 HALo|Check-In|Test|Check In - Items: YES | Attendance: NO | Backup: NO - Multiple Items - Booked in Advance - booked item not available', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9845')
    //login steps

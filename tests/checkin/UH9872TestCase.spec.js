...
test(' UH-9872 HALo|Check-In|Test|Override to Check In - Items: No | Attendance: Yes | Backup: No - Booked in Advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9872')
    //login steps

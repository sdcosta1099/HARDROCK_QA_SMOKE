...
test('UH-9851 HALo|Check-In|Check In for Overridden Booking: Items: Yes | Attendance: No | Backup: No | Items are available', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9851')
    //login steps

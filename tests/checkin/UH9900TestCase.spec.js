...
test('UH-9900 HALO|Check-In|[Y Y Y] Single Item | Booked In Advance - Attendance | fully committed | backup depleted', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9900')
    //login steps

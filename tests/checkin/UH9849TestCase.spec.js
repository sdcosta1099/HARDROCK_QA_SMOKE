...
test('UH9849 HALo|Check-In|Test|Override to Check In - Items: YES | Attendance: NO | Backup: NO - Multi item - Inv. Avail. - Not Booked in Advance ', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9849')
    //login steps

...
test('UH-9891 HALo|Check In|[Y Y N] Multi Item | Booked In Advance - Attendance | attendance available | primary depleted', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9891')
    //login steps

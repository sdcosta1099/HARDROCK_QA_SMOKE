...
test('UH-9917 HALO|Check-In|[Y Y Y] Multi Item | Booked In Advance - Backup | fully committed | backup available', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9917')
    //login steps

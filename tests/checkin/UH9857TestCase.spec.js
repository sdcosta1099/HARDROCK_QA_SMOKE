...
test(' UH-9857 HALo| Check In - Items: YES | Attendance: NO | Backup: YES - Single item - Inv. Avail. - Booked in advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9857')
    //login steps

...
test('UH-9865 HALo|Check in|Override to Check In - Items: YES | Attendance: NO | Backup: YES - Single Item - Booked in Advance', async ({ page }) => {
     
    const gblFuntSteps = new globalFUnctionSteps(page)
    const searchPlayerID= new playerSearchSteps(page)
    const configSteps= new configurationSteps(page)
    //generate random Offer name
    const offerName = gblFuntSteps.randomName('UH-9865')
    //login steps

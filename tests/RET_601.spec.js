...
test('RET_601, Display Unity points (checkout)', async () => {
    ...
    const common = new commonSteps(page)
        ...
        await page2.waitForTimeout(4000);
        await page2.locator('//button[@class="add-to-cart btn btn-primary"]').click();
        await page2.waitForTimeout(4000);
        await page2.getByRole('link', { name: 'Cart 1 Items' }).click();
        await page2.getByRole('button', { name: 'My Account' }).isVisible();
        await page2.locator('//div[@class="mobile-unit"]//span[contains(text(),"Remove")]').click();
        await page2.waitForTimeout(3000);
        await page2.locator('(//button[@class="btn btn-save btn-block cart-delete-confirmation-btn"])[1]').click();
        await page2.waitForTimeout(3000);
